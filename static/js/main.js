...
/**
 * Load and render address history list:
 *   /api/addr/{addr}/history?offset=&limit=
 */
// async function loadAddrHistory(addr, offset = 0, limit = 25) {
//   const j = await getJSON(
//     `/api/addr/${encodeURIComponent(addr)}/history?offset=${offset}&limit=${limit}`
//   );
//   const histEl = document.getElementById("addr-history");
//   if (!histEl) return;
//   if (!j) {
  ...
  const rows = j.items.map((it) => {
    ...
    return `
      <div class="list__item">
        ...
        <div class="mono mono-wrap">${it.txid}</div>
...
async function showTx(txid, resolveN = 20) {
  ...
  const voutList = vout
    .map((o) => {
      ...
      return `<li class="list__item"><div>${fmtNumber(
        val,
        8
      )} BTC → <span class="mono">${addr}</span></div></li>`;
    })
    .join("");
  ...
  const insList = ins
    .map(
      ...
      <li class="list__item">
        <div>
          ${fmtNumber(i.value_btc, 8)} BTC ← <span class="mono">${i.address}</span>
